use asteroids::{
    custom::{derive_setters::Setters, ElementTrait},
    ValidState,
};
use glam::{Quat, Vec3};
use mint::Vector3;
...
type OnGrab<State> = Box<dyn Fn(&mut State, Vector3<f32>) + Send + Sync>;
#[derive(Setters)]
...
impl<State: ValidState> std::fmt::Debug for GrabRing<State> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GrabRing").field("pos", &self.pos).finish()
    }
}
